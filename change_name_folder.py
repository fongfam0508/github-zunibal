...
def parse_folder_name(folder_name):
    ...
    if len(parts) == 6 and parts[0] in TINH_MAPPING and parts[1] in DAI_LY_MAPPING:
        return {
...
def update_fields():
    ...
    if data:
        tinh_var.set(data["tinh"])
        dai_ly_var.set(data["dai_ly"])
        so_tau_var.set(data["so_tau"])  # Cập nhật số tàu
        ma_thiet_bi_var.set(data["ma_thiet_bi"])  # Cập nhật mã thiết bị
        ma_niem_phong_var.set(data["ma_niem_phong"])  # Cập nhật mã niêm phong
        ngay_var.set(data["ngay"])  # Cập nhật ngày
    else:
        # Xóa các trường nếu tên thư mục không hợp lệ
        for var in [tinh_var, dai_ly_var, so_tau_var, ma_thiet_bi_var, ma_niem_phong_var, ngay_var]:
            var.set("")
...
def update_folder_name(*args):
    ...
    new_folder_name = f"{ma_tinh}.{ma_dai_ly}.{so_tau}.{ma_thiet_bi}.{ma_niem_phong}.{ngay}"
    entry.delete(0, tk.END)
